package com.appspot.kyusuisho.android.model;
...
import com.appspot.kyusuisho.android.activity.ActivityBase;
...
public class Area extends SQLiteOpenHelper {
	...
	private static final String CREATE_TABLE = "CREATE TABLE area ("
			+ "id VARCHAR(32) PRIMARY KEY," + "prefecture VARCHAR(32),"
			+ "city VARCHAR(32)," + "map_url TEXT," + "mobile_map_url TEXT,"
			+ "max_lat DOUBLE," + "max_lng DOUBLE," + "mid_lat DOUBLE,"
			+ "mid_lng DOUBLE," + "min_lat DOUBLE," + "min_lng DOUBLE,"
			+ "order_value INTEGER" + ")";

	public static void updateFromAPI(ActivityBase activity, Area db)
			throws URISyntaxException, ClientProtocolException, IOException,
			XmlPullParserException, KyusuishoException {
	...
	public static Vector<String> findPrefectures(ActivityBase activity, Area db) {
		...
		Cursor cursor = sqldb.query("area", new String[] { "prefecture" },
				null, null, "prefecture", null, "order_value");
	...
	public static Vector<String> findAreas(ActivityBase activity, Area db,
			String prefecture) {
		Vector<String> list = new Vector<String>();
		SQLiteDatabase sqldb = db.getReadableDatabase();
		Cursor cursor = sqldb.query("area", new String[] { "city" },
				"prefecture = ?", new String[] { prefecture }, null, null,
				"order_value");
		while (cursor.moveToNext()) {
			list.add(cursor.getString(0));
		}
	...
	public static String findMapUrl(ActivityBase activity, Area db, String city) {
		String url = "";
		SQLiteDatabase sqldb = db.getReadableDatabase();
		Cursor cursor = sqldb.query("area", new String[] { "map_url" },
				"city = ?", new String[] { city }, null, null, "order_value");
		while (cursor.moveToNext()) {
	...
	public static String findNearestMapUrl(ActivityBase activity, Area db,
			double lat, double lng) {
		String url = "";
		SQLiteDatabase sqldb = db.getReadableDatabase();
		Cursor cursor = sqldb.query("area", new String[] { "mid_lat",
				"mid_lng", "map_url" }, null, null, null, null, null);
		double minDistance = Double.MAX_VALUE;
		while (cursor.moveToNext()) {
			double clat = cursor.getDouble(0);
			double clng = cursor.getDouble(1);
			double distance = Math.pow(lat - clat, 2) + Math.pow(lng - clng, 2);
	...
	private static void parseList(InputStreamReader in, Area db)
			throws XmlPullParserException, IOException, KyusuishoException {
		...
		while (true) {
			...
			if (XmlPullParser.START_TAG == type) {
				...
				if (null != area && xmlTag.equals("area")) {
					String replace = "REPLACE INTO area VALUES ('" + area.id
							+ "','" + area.prefecture + "','" + area.city
							+ "','" + area.map_url + "','"
							+ area.mobile_map_url + "','" + area.max_lat
							+ "','" + area.max_lng + "','" + area.mid_lat
							+ "','" + area.mid_lng + "','" + area.min_lat
							+ "','" + area.min_lng + "','" + area.order_value
							+ "')";
